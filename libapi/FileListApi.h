...
namespace api 
{
class FileListApi : public fs::server::HttpHandler
{
public:
    FileListApi(std::shared_ptr<fs::service::FileService> const& fileService) {
        m_service = fileService;
    }
    ~FileListApi() {}

    void handleRequest(std::shared_ptr<fs::server::ServerRequest> request) override {
        int pageNum = std::atoi(request->getHeader("pageNum").c_str());
...
private:
    std::string   m_uri = FS_FILE_LIST_API;
