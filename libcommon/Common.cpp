...
std::string fs::common::getNowTime() {
    ...
    char timestr[TIME_FORMAT_LEN + 1];
    std::strftime(timestr, TIME_FORMAT_LEN + 1, "%Y-%m-%d %H:%M:%S", localTime);
...
std::string fs::common::encode(const std::vector<uint8_t>& data) {  
    std::string ret;  
    size_t i = 0;  
...
std::string fs::common::encode(const std::string& data) {  
    std::vector<uint8_t> vec(data.begin(), data.end());  
    return encode(vec);  
}  
  
std::vector<uint8_t> fs::common::decode(const std::string& encoded_string) {  
...
std::string fs::common::decode_to_string(const std::string& encoded_string) {  
    auto decoded = decode(encoded_string);  
